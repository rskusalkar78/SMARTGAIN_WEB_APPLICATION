...
import { format, startOfDay, parseISO, isWithinInterval, startOfWeek, endOfWeek, subDays, addDays } from 'date-fns';
...
export function WorkoutChart({ data, workoutPlan, isLoading }: WorkoutChartProps) {
    ...
    const { completionRate, weeklyData } = useMemo(() => {
        if (!workoutPlan || !(workoutPlan as any).workouts || (workoutPlan as any).workouts.length === 0) {
            ...
            let startDate = subDays(today, 6); // default to last 7 days

            if (safeData.length > 0) {
                const oldestLog = new Date(Math.min(...safeData.map(l => startOfDay(parseISO(l.timestamp)).getTime())));
                if (oldestLog < startDate) {
                    startDate = oldestLog;
                }
            }

            // Populate all days from startDate to today
            let curr = startDate;
            while (curr <= today) {
                dailyWorkouts.set(format(curr, 'yyyy-MM-dd'), 0);
                curr = addDays(curr, 1);
            }

            safeData.forEach(log => {
                const dateKey = format(startOfDay(parseISO(log.timestamp)), 'yyyy-MM-dd');
                if (dailyWorkouts.has(dateKey)) {
                    dailyWorkouts.set(dateKey, dailyWorkouts.get(dateKey)! + 1);
                } else {
                    dailyWorkouts.set(dateKey, 1);
                }
            });
