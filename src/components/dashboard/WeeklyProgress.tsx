...
const WeeklyProgress = ({ weightLogs, targetWeight }: WeeklyProgressProps) => {
  ...
  const chartData = dateArray.map((date, index) => {
    ...
    let weight = logForDate ? logForDate.weight : null;

    // Simulate a "zig zag" graph progression if there is only 1 or 0 real logs (e.g. new user)
    if (!logForDate && weightLogs.length <= 1 && targetWeight && activePlan) {
      if (date <= today) {
        const startWeight = weightLogs.length === 1 ? weightLogs[0].weight : (targetWeight - 5);
        const totalProgressDays = activePlan.userData.timeframe * 7 || 30;
        const currentDayIndex = index;
        
        // Base linear progress towards target
        const progressFactor = Math.min(1, currentDayIndex / totalProgressDays);
        const baseWeight = startWeight + (targetWeight - startWeight) * progressFactor;
        
        // Deterministic zig-zag noise (+/- roughly 0.5kg)
        const isUp = index % 2 === 0;
        const noise = isUp ? 0.4 : -0.2;
        
        weight = Number((baseWeight + noise).toFixed(1));
      }
    }
